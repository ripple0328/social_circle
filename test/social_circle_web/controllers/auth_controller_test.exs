defmodule SocialCircleWeb.AuthControllerTest do
  ...
  describe "account linking flow" do
    test "GET /auth/:provider/link/callback links additional provider to authenticated user", %{conn: conn} do
  ...
  describe "provider-specific routing" do
    test "all providers have consistent routing patterns" do
      ...
      for provider <- providers do
        ...
        {:ok, user} = create_test_user(:x, "routing_test_#{provider}_#{System.unique_integer([:positive])}@example.com")
        conn = build_conn() 
               |> init_test_session(%{})
